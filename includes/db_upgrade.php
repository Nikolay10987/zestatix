<?php
	...
	if ( empty( $current_db_version_zestatix ) || $current_db_version_zestatix < 101 ) db_101_zestatix();
	...
	function db_101_zestatix() {
		...
		$current_db_version_zestatix = 101;
	}

	if ( $current_db_version_zestatix < 102 ) db_102_zestatix();
	...
	function db_102_zestatix() {
	  ...
	  $charset_collate = 'ENGINE=InnoDB CHARSET=utf8 COLLATE=utf8_general_ci';
		...
		$current_db_version_zestatix = 102;
	}

	if ( $current_db_version_zestatix < 103 ) db_103_zestatix();
	...
	function db_103_zestatix() {
		...
		$current_db_version_zestatix = 103;
	}

	if ( $current_db_version_zestatix < 104 ) db_104_zestatix();
	...
	function db_104_zestatix() {
		...
		$current_db_version_zestatix = 104;
	}

	update_option( 'zestatix_db_version', $current_db_version_zestatix );
?>
